...
export default function SulHome({ navigation, route }) {
  const [fases, setFases] = useState([
    { numero: 1, bloqueado: false, concluida: false, imagem: require("../assets/Sul/Sudeste4.png") },
    { numero: 2, bloqueado: true, concluida: false, imagem: require("../assets/Sul/Sudeste1.png") },
    { numero: 3, bloqueado: true, concluida: false, imagem: require("../assets/Sul/Sudeste3.png") },
    { numero: 4, bloqueado: true, concluida: false, imagem: require("../assets/Sul/Sudeste2.png") },
  ]);
  ...
  useEffect(() => {
    const carregarProgressoSalvo = async () => {
      try {
        const progresso = await carregarProgresso('sul');
  ...
  // Atualiza fases com base no maior nível concluído
  ...
  // Atualizar quando receber fase concluída das telas de jogo
  useEffect(() => {
    const faseConcluida = route.params?.faseConcluida || 0;
    
    // Se não tem fase concluída ou já atualizou, sai
    if (!faseConcluida || jaAtualizouRef.current) return;
    
    jaAtualizouRef.current = true;
    
    // Calcula o máximo considerando apenas o que veio por parâmetro
    const maxConcluido = faseConcluida;
    
    if (maxConcluido > 0) {
      // Salvar progresso
      salvarProgresso('sul', maxConcluido);
      
      // Atualizar interface
      atualizarFases(maxConcluido);
      
      // Remove param para não atualizar novamente ao voltar
      const timer = setTimeout(() => {
        if (route.params?.faseConcluida) {
          navigation.setParams({ faseConcluida: undefined });
          jaAtualizouRef.current = false; // Reseta para próxima atualização
        }
      }, 100);
  ...
  const irParaJogo = (numeroFase) => {
    ...
    switch (numeroFase) {
      case 1:
        navigation.navigate("Minas");
        break;
      case 2:
        navigation.navigate("SaoPaulo");
        break;
      case 3:
        navigation.navigate("RioJaneiro");
        break;
      case 4:
        navigation.navigate("RioGrandeSul");
        break;
      default:
        break;
    }
  ...
  return (
    <View style={styles.container}>
      <Video
        source={require("../assets/Sul/fundo.mp4")}
