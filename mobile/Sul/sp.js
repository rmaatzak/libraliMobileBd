...
const letters = [
  { id: "mae", label: "MÃO", videoId: "h", image: require("../assets/Sul/mao.png") },
  { id: "pai", label: "PÉ", videoId: "j", image: require("../assets/Sul/pe.png") },
  { id: "irmao", label: "BOCA", videoId: "k", image: require("../assets/Sul/boca.png") },
  { id: "vovo", label: "NARIZ", videoId: "l", image: require("../assets/Sul/nariz.png") },
  { id: "avo", label: "OLHO", videoId: "m", image: require("../assets/Sul/olho.png") },
];

const VIDEOS_LIBRAS = {
  h: require("../assets/Sul/mao.mp4"),
  j: require("../assets/Sul/pe.mp4"),
  k: require("../assets/Sul/boca.mp4"),
  l: require("../assets/Sul/nariz.mp4"),
  m: require("../assets/Sul/olho.mp4"),
};
...
export default function SaoPaulo({ navigation }) {
  ...
  async function salvarProgresso() {
    try {
      await AsyncStorage.setItem("sao_paulo_concluida", "true");
      console.log("Progresso salvo: São Paulo concluído");
    } catch (error) {
  ...
  const handleReloadVideo = async () => {
    ...
    if (videoRef.current) {
      try {
        ...
        console.log("Erro replay:", err);
        // Tenta recarregar
        await videoRef.current.unloadAsync();
        await videoRef.current.loadAsync(VIDEOS_LIBRAS[letters[currentLetterIndex].videoId]);
        await videoRef.current.playAsync();
      }
    }
  };
  ...
  const handleReloadGameVideo = async () => {
    ...
    if (gameVideoRef.current) {
      try {
        await gameVideoRef.current.replayAsync();
      } catch (err) {
        console.log("Erro replay jogo:", err);
        await gameVideoRef.current.unloadAsync();
        await gameVideoRef.current.loadAsync(VIDEOS_LIBRAS[gameOrder[currentGameQuestion].videoId]);
        await gameVideoRef.current.playAsync();
      }
    }
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      {screen === "overlay" && (
        <Animated.View style={[styles.overlay, { opacity: overlayFade }]}>
          <Text style={styles.overlayText}>SÃO PAULO</Text>
        </Animated.View>
      )}
        ...
        <ImageBackground source={require("../assets/Sul/sp.png")} style={styles.bg}>
          <View style={styles.frameBorder}>
            <View style={styles.center}>
              ...
              <Image source={require("../assets/Sul/verde.png")} style={styles.character} />
            </View>
          </View>
        </ImageBackground>
      )}
      ...
      {screen === "finalizado" && (
        <View style={styles.finalizadoContainer}>
          ...
          <View style={styles.center}>
            ...
            <Animated.Image source={require("../assets/Sul/Sudeste1.png")} style={[styles.polaroid, { opacity: fadeAnim }]} />
            ...
            <View style={styles.finalButtonsRow}>
              ...
              <TouchableOpacity
                ...
                onPress={() => navigation.navigate("SulHome", { faseConcluida: 2 })}
              >
                <Image
                  source={require("../assets/Sul/certo.png")}
                  style={styles.iconButton}
